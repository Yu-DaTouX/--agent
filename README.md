...
| 看会话分支 | 左栏会话行上直接标：父会话显示**分支数** `⑂ N`，子会话显示**编号** `#N` 与 `从「<那句话>」分出去`；点分支会话行即切过去。**分支创建只在对话窗口里**（用户消息上的「分支」） |
| 看模型的推理 | 回答上方的**推理窗口**：固定约 1/4 屏高、推理逐字在里面滚动；结束后折叠成一行（点标题可再打开）。没有推理就不显示 |
| 工具调用详情 | 一行一条（Codex 风格）；点开是**终端窗口**。要不要默认展开见设置 → 外观 → 工具调用详情 |
...
| `branch` | 会话分支（左栏）：无分支按钮 / 分支数 `⑂N` / 子会话编号 `#N` / 分叉自哪句话 / 点进分支会话 | 否 |
