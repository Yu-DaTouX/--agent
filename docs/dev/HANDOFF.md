...
>
> 最后更新：2026-09-13（**第 22 版：左栏排序/标题、导航轨重写、标题栏精简、删除按钮可用**）
>
...
做一个 **独立的 agent 桌面端**（产品名「**砚**」）：Electron 外壳 + React 渲染层，
中文界面 + 完整 i18n 通道，**类终端调性**（全等宽、无投影、颜色即语义），可开源分发。

**定位：砚 是 agent，pi 只是内核。**
砚自己拥有会话与项目、输入与命令、工具调用的呈现、任务/状态/日志、模型接入、打包分发；
pi（`pi --mode rpc` 子进程）只负责模型循环与工具执行。**不往 pi 里塞东西**：
不打补丁、不 import 内部模块、不写用户的 extensions 目录。

> 变更：原来的「记忆是第一公民」（remember/recall/forget + 提示词注入 + 认识论面板）
> 已按用户要求**整体移除**。旧的 `~/.pi/agent/yan/memory.json` / `soul.md` 不主动删。

**当前阶段：功能完整、可日常使用，且已能打包分发。**
...
| **阶段 2.8：任务模块与窄窗复核** | ✅ **完成**（2026-09-12）—— 任务「正在进行」就地显示 / 18 字 / **历史任务折叠 + 跳转** / 导航轨与压缩记号 / 窄窗口三档复核 |
| **阶段 2.9：开关归位（参考 Codex）** | ✅ **完成**（2026-09-12）—— 两个面板开关回**标题栏两端**（收起 = 0 宽）/ 修**导航轨错位** / 左栏「砚 ⌄」**模式菜单入口** |
| **阶段 2.10：对话结构与代码结构** | ✅ **完成**（2026-09-12）—— 推理胶囊（逐字流式）/ 工具调用 **Codex 一行式** + 终端详情开关 / **分支树**（左栏）/ components 按 chat·rail·toolbar·settings·shell 分组 |
| 阶段 3：打包分发 | ✅ **完成**（2026-09-13）—— electron-builder（NSIS 安装包 + 免安装单文件版）/ 内置 pi 走 extraResources / 应用图标 / **打包验收探针** |
| 记忆移除 + 定位重写 | ✅ **完成**（2026-09-13）—— 删干净 remember/recall/forget + 提示词注入 + 认识论面板；定位改为「独立 agent，pi 内核」（详见 §8.18 第 27 条）|
...
| 10 | ~~**字体子集方案**~~ | ✅ **已解决**：改用 npm 包 [`@mogeko/maple-mono-cn`](https://www.npmjs.com/package/@mogeko/maple-mono-cn)（cn-font-split 重打包，239 个 woff2 分片约 9.7MB，随 Vite 打进 `out/`），`npm install` 即就位 | ✅ 已定 |
...
| 9 | **编辑脚本吞引号/吞文件尾** | 我用 node 脚本做批量替换时，引号被吃掉（`typeof v === number`）、JSDoc 里的 `# 交接文档 · pi desktop` 被当成替换模式（首行变成 `NaN`）、整段函数尾被删。**含引号/注释的代码改动一律用编辑工具，不要拼 shell 字符串** |
| 10 | **别写死「外部工具」的常量** | 压缩阈值不是 16384 而是 `contextWindow - reserveTokens`，且 reserveTokens 用户可配（pi 的 settings.json）。**读它的配置，不要抄一个数字** |

### 8.14 第 15 版的坑（2026-09-12：任务模块 / 收起槽 / 窄窗口）

| # | 坑 | 教训 |
|---|---|---|
| 1 | **「隐藏条」与「按钮对齐」是一对矛盾要求** | 8px 的缝放不下 26px 的按钮 → 按钮只能错位；38px 的槽能对齐但「宽」。**解法是把「隐藏」理解为视觉性质（透明 + 无边框），而不是宽度为 0** —— 诉求里往往有一条是真正的目标，另一条是实现手段 |
| 2 | **同一条规则写两处** | `.workspace:not(:has(.rightpanel)) { --w-right }` 在 §23b 与 §54 各有一份（0px vs 38px），后者生效、前者是死代码。读代码的人会以为收起是 0 宽。**CSS 变量/关键规则只在一处定义** |
| 3 | **设计换了但断言没换** | 收起宽度 8 → 38 之后，resize / symmetry / panels / narrow 四个探针里都有 `≤12px` 的断言。**断言要绑设计意图（没有可见的条 / 入口可点 / 对齐），不要绑某一版的像素值** |
| 4 | **只留最新一份 = 拿不到历史** | `todosFromEntries` 只保留最后一份清单，于是「历史任务」在数据层就不存在（不是 UI 没画）。**要做历史/版本，数据层就得先保留** |
| 5 | **跳转需要「属于第几轮」这个信息** | 快照只存 todos 是不够的 —— 要数它前面有多少条用户消息（`round`）才能跳回那次对话。**做「跳转」前先确认数据里有可定位的锚** |
| 6 | **窗口尺寸渲染端改不了** | `window.resizeTo` 对非脚本打开的窗口无效 → 窄窗口的 bug 只能用 `YAN_WIN=宽x高` 从主进程控制（探针加环境变量）。**有些测试条件必须从主进程造** |
| 7 | **注释里的旧数字也是坑** | 收起槽从 8 改成 38 后，注释仍写着「22px 按钮比 8px 的缝宽」——**改数值时一并改掉解释它的注释**，否则下一个人会按注释去「修」回 8px |

### 8.15 第 16 版的坑（2026-09-12：开关归位 / 导航轨错位）

| # | 坑 | 教训 |
|---|---|---|
| 1 | 「错位」要量**相对位置**，不是量绝对坐标 | 导航轨钉在中栏左缘，而消息列是 max-width **居中**的 —— 中栏比内容列窄时两者重合（看不出问题），一收起左栏中栏变宽就错位 110px。**报「错位」时先找「两个本该对齐的东西各自的参照物是什么」** |
| 2 | **在面板内部放开关是个陷阱** | 面板一收起，开关跟着塌陷 → 必须给收起态**保留宽度**（38px）→ 那条保留宽度改变了中栏宽度 → 又把别的对齐算错。**把「开关」与「被控制的面板」解耦（放标题栏）比在面板内部想办法省事得多**。用户的两条要求（隐藏条 / 按钮对齐）在面板内只能靠 38px 透明槽折中，移到标题栏后两条同时消失 |
| 3 | **断言绑像素值 → 改设计就假失败** | 这一轮 5 个探针（layout / resize / narrow / panels / symmetry）都断言了「开关在面板内 / 收起槽 38px / 有悬停入口」—— 全是实现细节。改成断言**意图**（开关在标题栏 / 收放时不动 / 收起=0 宽 / 入口点得到）后，下次再改版也不会假失败 |
| 4 | **清理旧样式时 `cut()` 打偏，删掉了一整节** | 用「找分隔注释 → 删到下一个分隔注释」的方式清理，结果把 §50（宽度把手 .resizer）整节删了。症状：把手变成占满整栏的隐形层（拖拽还能用，但位置断言量到 300px）。**删样式后要核对 git diff 里的被删选择器列表** |
| 5 | **多层转义把 `\n` 变成真换行** | 用 node -e 拼探针代码时转义层层衰减，`\n` 落进文件成了实际换行 → 探针语法错误（Invalid or unexpected token）。**改代码用编辑工具**（这条已记过两次，这是第二次） |

### 8.16 第 17 版的坑（2026-09-12：对话结构 / 代码整理）

| # | 坑 | 教训 |
|---|---|---|
| 1 | **批量改代码的脚本会吃掉函数** | 我用「找函数名 → 删到顶格 }」的思路清理死代码，行号在每次删除后位移 → 把 **ToolDetail 还要用的 `extractEdits` 也删了**（typecheck 立刻报出来）。**删代码用编辑工具逐个来**；脚本只适合「已经确定边界」的机械替换 |
| 2 | **同一份关注点漂成四代 CSS** | `.tool`（工具卡）在 redesign.css 里有 **4 处**定义（卡片 → 方角 → 一行 → 我新加的 .trow）。找边界时要一路看到底，否则删一半留一半更乱。**样式只在一节定义**（现在 `.trow/.tgroup/.term` 都在 §61） |
| 3 | **扫 i18n 孤儿键能发现「功能不可达」** | 扫出 `rail.rename` 没人用 → 顺着往下查：store 的 `renameSession`、主进程 IPC、pi 的 `set_session_name` **三层都通，但界面没有任何入口**。**「没人用的文案」可能是缺入口，不只是多余文案** |
| 4 | **一行的工具默认展开会顶走回答** | Codex 默认收起（点开才看详情）。所以 `toolDetail` 这个开关的默认值必须是 **false** —— 一次 agent 跑几十条命令是常态 |
| 5 | **推理输出 ≠ 思考档** | `thinkingLevel` 是设置（用多大强度推理），推理胶囊显示的是**模型实际吐出的推理文本**。两种都要有，且「没有推理就不显示」 |
| 6 | **协议没有 navigate_tree** | 47 个命令里没有切分支的命令 —— 分支树只能「查看 + 从这里分支」，**不能**「切到已存在的分支」。界面不许有这个按钮（不做假功能） |
| 7 | **逐字输出要跟得上** | 模型按**块**送（一次几百字）。固定「每帧 1 字」会越落越远，所以速率跟积压量走（`clamp(积压/8, 1, 24)`），稳定落后约 130ms |
| 8 | **探针断言不要绑「碰巧的输入」** | branch 探针原来按固定文件名找 fixture（测试的 fixture 集合会变）→ 改成「逐个试，直到某个会话真有分支点」；todos 原来拿「第一个不是当前的会话」当「无任务会话」→ 改成逐个试。**单跑必过、全量才炸的第三种成因** |
| 9 | **DOM 改了，探针要跟着改** | 工具卡 → 一行式（`.tool` → `.trow`）后，features 探针里 4 处选择器全失效（其中一处藏在等待循环里）。**改 DOM 结构后先搜一遍 probe 里的类名** |

### 8.17 第 18 版的坑（2026-09-13：Windows 打包分发）

| # | 坑 | 教训 |
|---|---|---|
| 1 | **electron-builder 会跳过被拷目录的顶层 `node_modules`** | `extraResources` 写 `from: resources/pi-runtime` 时，内置 pi 的 6 个最小依赖（`@earendil-works/chord` 等）**静默丢失**。源码是 `app-builder-lib/out/util/filter.js` 的 `if (relative === "node_modules") return false`（它以为那是 app 根的依赖，交给依赖解析）。**解法：把 `node_modules` 单列一条 `from`** —— 相对路径不再等于 `node_modules`，规则不触发。症状：开发态场景全绿、装出来连不上 pi |
| 2 | **`files: ['!node_modules/**']` 是 asar 瘦身的关键** | 不排除的话 electron-builder 会把 `react`/`react-markdown` 那一坨（28.9MB）打进 `app.asar`，而渲染层依赖其实已被 Vite 打进 `out/`。排除后 asar **42.9MB → 11.6MB** |
| 3 | **portable 单文件版没有可用的 stdout** | electron-builder 的 portable 外层包装**不转发子进程 stdout**，用 `YAN_PROBE` 抓输出会「一个字节都没有」，看着像启动失败。**探针结果要能写文件**（新增 `YAN_PROBE_OUT`），别只靠 stdout —— Windows GUI 应用本来就不保证有控制台 |
...
| 11 | **模型默认用英文推理** | 中文提问、`thinking` 内容是英文（实测裸 RPC）—— 推理窗口一显示就是「中文提问 + 英文旁白」，很割裂。在记忆扩展的 `before_agent_start` 里读 `desktop.json` 的 `lang` 注入语言指令。**读文件而不是环境变量**：语言在设置里随时可改，env 只在 pi 启动时读一次 |
| 12 | **右缘的两个东西靠 `:hover` 同时显现 = 叠成一团** | 会话行右缘的**相对时间**与**动作按钮**（绝对定位）都在悬停时显示，画在同一位置（用户截图里那团重影）。规则改成：带动作的行（选中）悬停时**时间让位**。`:hover` 类断言测不了，只能验「CSS 规则存在」（本次就是这么做的） |
| 13 | **跨会话分支的数据在 `session` 头里** | `parentSession` 指向分叉来源的会话文件；而分叉时 pi 会把父会话的条目**拷到子会话开头**（保留原时间戳，`session` 头的 timestamp 就是分叉时刻）—— 所以只读子会话就能算出「从哪句话分出去」（扫到时间戳 ≥ 分叉时刻为止，最后一条用户消息）。⚠️ 扫的时候**要跳过第 0 行**（那是 session 头，它自己的 timestamp 就是分叉时间，不跳就立刻 break） |
| 14 | **删掉一条链路要连根拔** | 分支改成会话级后，会话内的 `get_tree` 分支树整条链路（`BranchTree.tsx` / `session-tree.ts` / agent 方法 / IPC / preload / 类型 / 一段 CSS）都成了死代码，一并删掉。**删完 grep 一遍旧符号**（`SessionTree|trimTree|BranchTree` → 0）|
| 15 | **排序键选错 = “打开就置顶”** | 用户报「进入会话的时候不要置顶」。列表以前按 `updatedAt`（= 文件 mtime）排，而**打开会话会写会话文件**（标题生成 / name 落盘）→ mtime 刷新成“刚刚” → 那一行跳到最上面。**要区分“创建时间”与“最近一次写盘”**：排序用不会因“打开”而变的 `createdAt` |
| 16 | **`<button>` 不能嵌 `<button>`** | 分叉开关要求放在「会话标题旁」，而会话行本身就是 `<button>` —— 嵌进去浏览器会把内层拆出去、行为不可预期。拆成 `.srow-row = [.srow 按钮][.srow-btoggle 按钮][时间]`。**要加交互元素时先看父节点是不是 button/a** |
| 17 | **排序键要选「不会因打开而变」的** | 「进入会话就置顶」→ 换 `createdAt` 后用户又说「很乱」（`createdAt` 是**分叉时刻**，子会话全挤到顶）。正确做法：用**最后一条 message 的时间**（读文件尾部 64KB）——打开会话只写 `session_info`/标题，不动 message，所以稳定又是真实的「最近活动」。 |
| 18 | **左栏混排很乱** | 分叉出来的子会话散落在各处。按**家族分组**：根会话按最近活动倒序，子会话紧随它的父（同族按创建先后）。 |
| 19 | **CSS 两节打架 + 类名脱节** | 导航轨的 JSX 类名是 `.outline-hit` / `.outline-bar`，而旧节的规则写的是 `.outline-tick`（全死）；两节的 `.outline` 又互相覆盖；`op-title` / `op-answer` / `op-empty` **根本没有样式**。**改 CSS 前先 grep 类名是否真的存在**；一类关注只留一节。 |
| 20 | **“随窗口缩放”纯 CSS 做不到** | 导航轨要贴在**居中限宽**的内容列左边；窗窄时内容占满、左边只剩 24px 内边距，而悬停刻度要长到 34px —— CSS 没有“有边距就放外面、没边距就夹进内边距”的分支。改成 **JS 实测内容列左缘**（`ResizeObserver`）再设 `left`。 |
| 21 | **动作按钮“只在选中行渲染”+“删除又禁用选中行” = 删除永远不可达** | `.srow-acts` 以前只在 selected 行渲染，而菜单里的删除又 `disabled={selected}` → 任何行都删不了（用户报的）。现在**每行都渲染 ⋯**、悬停才显形；隐藏时 `pointer-events:none`，否则看不见的按钮会抢走“点行选中”。 |
| 22 | **标题 8 字上限** | 用户报「会话标题显示字很少」——`TITLE_MAX = 8` 写不出信息量（“查看图片”“网线治丢包”）。提到 18，提示词同步改。 |
| 23 | **删掉标题栏中段要给 grid 留占位** | 删了「已连接 · 工作目录」后，`.titlebar` 是三列 grid（auto/1fr/auto）——少一个子元素右侧那组会掉进中列被拉宽。留一个空的 `.tb-center`。 |
| 24 | **展开规则写反了** | `open = detailOn && !running` —— 打开「工具详情」开关后，**已结束的**每一条都展开成终端窗口，正在跑的那条反而收起。应改为：**只有正在运行的那条自动展开**，其余保持一行（要看详情自己点）。 |
| 25 | **`/login` 在 RPC 模式下不存在** | pi 的订阅制登录是**交互式 OAuth**（要开浏览器、回调回连 localhost），47 个 RPC 命令里没有 login —— 直接发过去模型会当成一句话回答（用户报的「/login 没办法正常接受」）。在 Composer 里**拦截并路由到「模型接入」窗口**。 |
| 26 | **显示的时间要和排序键一致** | 排序用 `lastActivityAt`（最后一条 message），而界面显示的是 `updatedAt`（mtime）——两者不一致时列表看起来“没排序”。显示与排序必须用同一个键。 |
| 27 | **“记忆”整体移除的做法（可复用的路径）** | 一个横跨 30 个文件的子系统怎么拆干净：① 先断**数据来源**（不再给 pi 传 `--extension` → 工具/提示词注入一起消失）② 再拆**主进程**（`memory.ts` 只剩路径常量 → 改名 `paths.ts`；删 IPC/preload/类型）③ 再拆**渲染层**（面板/审阅条/空状态/设置页/store）④ 最后**探针/打包配置/文档**。⚠️ i18n 是**类型安全**的：`t('mem.xxx')` 还在调用处而键没了 → 编译期报错（这正是好处）；删组件后要连键一起删。⚠️ 用户数据文件（memory.json/soul.md）**不删**。 |
| 28 | **“任务面板”是展示，不是指引** | 用户指出：面板应该**展示模型的工作**，不应该**指引模型**。查证：应用侧本来就只是**读**会话里的 `custom` entry（`panel_todos` 写的），从不注入任何东西；真正的“指引”在**用户自己的扩展**里 —— 导出的系统提示词里那两行（`- panel_todos: …` 与 `- Use panel_todos to keep the task list … set it when you plan …`）就是它。所以“去掉指引”要么改用户的扩展描述，要么把面板改成从**会话活动**推导。 |
...
| 7 | ~~**字体子集方案**~~ | ✅ **已解决**：npm 包 `@mogeko/maple-mono-cn`（239 个 woff2 分片，约 9.7MB） |
...
## 10.2 Windows 打包分发（已落地，2026-09-13）

配置：[`electron-builder.yml`](../electron-builder.yml)（根目录）。命令 `npm run dist`。

| 产物 | 说明 |
|---|---|
| `release/砚-<版本>-setup.exe` | NSIS 安装包（约 120MB；可选目录、桌面/开始菜单快捷方式） |
| `release/砚-<版本>-portable.exe` | 免安装单文件版（约 120MB，自解压到临时目录） |
| `release/win-unpacked/` | 免安装目录（约 399MB；`npm run dist:dir` 产出，调试用） |

**随包资源（extraResources → `<安装目录>/resources/`）**

| 从 | 到 | 谁在用 |
|---|---|---|
| `resources/pi-runtime/dist` + `…/node_modules` + `…/package.json` | `pi-runtime/…` | `protocol.ts` 的 `bundledRoots()` 读 `process.resourcesPath/pi-runtime` |
| `resources/pi` | `pi/` | `index.ts` 的 `extensionPath()` 读 `process.resourcesPath/pi/yan-memory.ts` |

> ⚠️ `node_modules` 必须**单列一条 `from`**（见 §8.17 第 1 条），否则内置 pi 的依赖静默丢失。

**图标**：`npm run icon` → `build/icon.ico`（16/32/48/64/128/256）+ `build/icon.png`（512）。
图案是深色圆角方块 + 青色四角星（对话里「砚」的角色符号 ✦）；不用「砚」字，
因为任务栏最小 16px 下汉字会糊。`build/` 只白名单放行这两个文件。

**打包验收**：`npm run test:packaged`（或 `npm run dist:check` = `dist:dir` + 验收）。
跑 `release/win-unpacked/砚.exe` 里的**真实应用**，断言：
`conn=ready` / `piInfo().bin` 落在包内 `resources/pi-runtime/…/cli.js`（没退回全局 pi）/
报到 pi 版本号 / `get_commands` 有返回 / 无连接失败条。
`--exe=<路径>` 可直接验 portable 单文件版（走 `YAN_PROBE_OUT` 写文件）。

**已知未做**：代码签名（安装会触发 SmartScreen 提示）、macOS/Linux 打包。
字体已不必再子集化（npm 包 239 个 woff2 分片，约 9.7MB）。
...
| # | 任务 | 为什么 |
|---|---|---|
| 1 | ~~**打包分发**~~ | ✅ **已完成**（2026-09-13）：electron-builder → NSIS + portable，见 §10.2；剩代码签名与 macOS/Linux |
| 2 | **内置 pi 的功能完善**（用户指定优先） | 运行时已内置且能跑，但「内置」这件事还没有正经界面与升级通路：① 设置→状态/关于里说清「当前用哪个 pi」（内置/外部 + 路径 + 版本 + 重新检测）② `vendor:pi:check` 接进 `check` ③ pi 升版后的重跑流程 / 应用内提示 ④ 内置缺件时的明确提示（不是“未连接”） |
