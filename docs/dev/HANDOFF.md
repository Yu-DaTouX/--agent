...
| 方案 | 文件 | 要拍板的点 |
|---|---|---|
| 自动压缩收进上下文工具 | [方案 · 上下文工具内的自动压缩（v2）](../design/方案-上下文工具内的自动压缩-2026-09-15.md) | 已按用户批注修订：阶段 1 不再提前显示工作集（仍忠实显示物理窗口），阶段 2 优先用 RPC `compaction_*` 事件（无需 extension），阶段 3 拆成“砚决策 + pi 扩展执行”，工作集公式含输出预留与安全余量，并重排为阶段 0–4。待确认：阶段 0 接口草案、阶段 1 是否开工 |
