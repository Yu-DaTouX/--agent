...
>
> 最后更新：2026-09-12（**第 12 版：内置 pi / 工具栏（文件树+日志）/ 用户档案 / 界面缩放 / 启动器**）
>
...
| # | 任务 | 说明 |
|---|---|---|
| D | 引导第二步：检查**本地已有 key** | 现在只读 `auth.json`，**环境变量配的（OPENAI_API_KEY 等）完全没算进去**；另需「我已配好 → 重新检测」按钮 |
| E | 左右栏宽度可拖拽 | 落盘 + 双击复原；键盘可调（无障碍） |
| F | 工具栏分区可拖拽排序 | 建议用**指针事件**（文件树已用过同一套），不用 HTML5 DnD |
| G | 工具库按钮 | 分区收进库 / 从库取回；与 F 共用 order + hidden 两份数据；库放工具栏头部 |
