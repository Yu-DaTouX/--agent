...
| `npm run test:packaged` | 验已有目录包，检查包内 pi 与扩展 |
...
单文件版每次启动会自解压；ZIP 版解压一次后运行 `砚.exe`。Windows 包内包含 pi runtime 和自有扩展，最终用户无需安装 pi。
...
3. 执行 dist:dir 和 test:packaged；核对包内运行时、扩展和路径，不用开发态测试替代包内验收。
