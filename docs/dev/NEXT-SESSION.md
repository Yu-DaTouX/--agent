...
已确认：任务面板保持现状；记忆模块和提示词注入已移除，不恢复；会话树旧链路是主动移除，不自动补回。主题、设置、模型接入和 Windows 打包均已有实现。内置浏览器与本机 Chrome 接入已完成，二者通过统一标签栏共存（`activeMode` 路由，外部标签 ID 为 `chrome:<targetId>`）；内置 pi 管理/升级、运行时校验入总检查、Ctrl+Shift+P 上一模型均已完成。
