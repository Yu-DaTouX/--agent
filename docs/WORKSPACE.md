...
- 经用户授权深度清理，已删除 `.backup/` 旧快照；后续新备份仍需单独确认，不能视作缓存。
- 经用户授权已删除 `release/` 旧发布包、`out/` 和根目录 TypeScript 增量缓存。构建与单元测试已在清理前通过；启动入口会按需重建，手动测试前请先运行 `npm run build`。
...
- 类型检查启用 `noUnusedLocals` 和 `noUnusedParameters`，阻止未使用声明继续积累。
- 分节使用简短的 `/* 分节名称 */`，不保留大段装饰分隔线；类型和公共接口说明保留 JSDoc。
- 注释说明当前职责、边界条件和设计原因，不记录“本次改动”等临时流水账。
- 保留安全限制、兼容性原因、单位、编译指令与许可证；第三方依赖及内置运行时不参与注释整理。
- 本轮扫描 147 个自有源码文件，统一了 459 处分节注释，并修正共享协议、右侧面板和样式入口的过期说明；不代表逐条重写了全部业务注释。
- 保留仍被使用的样式层、测试探针、设计档案与字体资料，不凭文件名判定无用。
